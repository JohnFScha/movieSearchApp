...
const MovieListContainer = ({
  ...
  return (
    <section>
      <div className={token ? "grid grid-cols-4 gap-10 m-5" : "flex gap-10 m-5"} ref={token ? parent : null}>
        {movies.map((movie) => {
          return (
            <article
              key={movie.id}
              className={
                token
                  ? "flex flex-col justify-between border border-solid border-gray-500 rounded-md shadow-lg shadow-slate-800 bg-cyan-900 text-white"
                  : "flex flex-col justify-between border border-solid border-gray-500 rounded-md shadow-lg shadow-slate-800 bg-cyan-900 text-white w-60"
              }
            >
